...
final class AdminOrderRepository
{
    ...
    public function updateBankStatus(int $orderId, string $bankDep): bool
    {
        $pdo = Database::connection();
        if (!$pdo instanceof PDO
            || $orderId <= 0
            || !$this->hasTable('sales_master')
            || !$this->hasSalesMasterColumn('s_id')
            || !$this->hasSalesMasterColumn('bank_dep')
        ) {
            return false;
        }

        $allowed = ['0', '1', '2'];
        if (!in_array($bankDep, $allowed, true)) {
            $bankDep = '0';
        }

        $statement = $pdo->prepare(
            'UPDATE sales_master
             SET bank_dep = :bank_dep
             WHERE s_id = :order_id
             LIMIT 1'
        );
    ...
    public function updateShipping(int $orderId, array $input): bool
    {
        $pdo = Database::connection();
        if (!$pdo instanceof PDO
            || $orderId <= 0
        ...
        if ($this->hasSalesMasterColumn('unsou')) {
            $assignments[] = 'unsou = :unsou';
            $params[':unsou'] = trim((string) ($input['unsou'] ?? ''));
        }

        if ($this->hasSalesMasterColumn('yamato_num')) {
            $assignments[] = 'yamato_num = :yamato_num';
            $params[':yamato_num'] = trim((string) ($input['yamato_num'] ?? ''));
        }

        if ($this->hasSalesMasterColumn('yamato_bikou')) {
            $assignments[] = 'yamato_bikou = :yamato_bikou';
            $params[':yamato_bikou'] = trim((string) ($input['yamato_bikou'] ?? ''));
        }

        if ($this->hasSalesMasterColumn('mail_flag')) {
            $shipmentState = (string) ($input['shipment_state'] ?? 'pending');
            $assignments[] = 'mail_flag = :mail_flag';
            $params[':mail_flag'] = $shipmentState === 'shipped' ? '1' : '0';
        }
